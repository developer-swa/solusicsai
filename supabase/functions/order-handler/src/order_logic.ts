...
export async function handleOrderingLogic(
  ...
  switch (cart.step) {
    ...
    case 'awaiting_payment':
      ...
      return await handleAwaitingPaymentState(supabase, userId, whatsappUserId, message, cart)
...
async function handleCollectingItemsState(
  ...
  if (foundItems.length > 0) {
    ...
    // Check if the message is clearly ordering-related before responding
    const orderingKeywords = [
      'menu', 'pesan', 'order', 'beli', 'mau', 'ingin', 'saya', 'aku',
      'makanan', 'minuman', 'sushi', 'ramen', 'roll', 'nigiri', 'gyoza',
      'porsi', 'gelas', 'buah', 'pcs', 'tambah', 'lagi'
    ]
    
    const isOrderingRelated = orderingKeywords.some(keyword => 
      lowerMessage.includes(keyword)
    )
    
    if (isOrderingRelated) {
      // Only respond with menu guidance if the message seems ordering-related
      return {
        response: "Maaf, saya tidak menemukan menu yang Anda maksud. Silakan ketik nama menu yang tersedia atau ketik 'menu' untuk melihat daftar lengkap. 🍣"
      }
    } else {
      // Return empty response to let general AI handle non-ordering questions
      return {
        response: ""
      }
    }
...
async function handleCollectingDetailsState(
  ...
  // Check if this is a general question unrelated to providing order details
  const detailsKeywords = [
    'nama', 'hp', 'nomor', 'telepon', 'outlet', 'cabang', 'ambil', 'antar', 
    'delivery', 'pickup', 'alamat', 'saya', 'aku', 'palagan', 'malioboro'
  ]
  
  const isDetailsRelated = detailsKeywords.some(keyword => 
    lowerMessage.includes(keyword)
  )
  
  // If message doesn't seem related to providing order details, let general AI handle it
  if (!isDetailsRelated && !lowerMessage.includes('lanjut') && !lowerMessage.includes('selesai')) {
    return {
      response: ""
    }
  }
...
async function handleConfirmingOrderState(
  ...
  // Check if this is a confirmation-related message
  const confirmationKeywords = [
    'ya', 'benar', 'lanjut', 'bayar', 'oke', 'ok', 'setuju', 'konfirmasi',
    'tidak', 'batal', 'cancel', 'salah', 'ubah', 'ganti'
  ]
  
  const isConfirmationRelated = confirmationKeywords.some(keyword => 
    lowerMessage.includes(keyword)
  )
  
  // If message doesn't seem related to order confirmation, let general AI handle it
...
async function handleAwaitingPaymentState(
  ...
  const paymentKeywords = [
    'bayar', 'pembayaran', 'lunas', 'sudah', 'selesai', 'transfer', 'qr', 'gopay',
    'menu', 'pesan', 'order', 'batal', 'cancel'
  ]
  
  const isPaymentRelated = paymentKeywords.some(keyword => 
    lowerMessage.includes(keyword)
  )
  ...
  if (isPaymentRelated) {
    ...
    // Return empty response to let general AI handle non-payment questions
    return {
      response: ""
    }
  }
